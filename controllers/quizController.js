...
export const getQuizzes = async (req, res) => {
  try {
    ...
    // Fetch user profile to get selectedStack
    let userStack = null;
    if (userId) {
      const profile = await StudentProfile.findOne({ userId });
      if (profile) {
        userStack = profile.selectedStack;
      }
    }
    ...
    // If specific stack requested via query, use that
    if (stack) {
        filteredQuizzes = allQuizzes.filter(q => q.stack && q.stack.toLowerCase() === stack.toLowerCase());
    } 
    // Otherwise if user has a stack, show quizzes relevant to that stack
    else if (userStack) {
        filteredQuizzes = allQuizzes.filter(q => 
            q.stack && userStack.toLowerCase().includes(q.stack.toLowerCase())
        );
    }
